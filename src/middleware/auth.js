...
//     if (user.role !== "admin" && user.role !== "vendor") return next(new ErrorHandler("You are not authorized to perform this action", 400))
//     return next()
...
export const adminOnly = TryCatch(async (req, res, next) => {
    ...
    if (Role !== "admin") return next(new ErrorHandler("You are not authorized to perform this action", 401));
    req.user = decodedToken
    return next()
})


export const authAdminMiddleware = TryCatch(async (req, res, next) => {
    const authHeader = req.headers['authorization']; // or req.get('Authorization')
    if (!authHeader) return next(new ErrorHandler("Please login first", 401));
    ...
    const Role = decodedToken.role
    console.log('role', Role)
    if (!(Role === "admin" || Role === "vendor")) return next(new ErrorHandler("You are not authorized to perform this action", 401));
    req.user = decodedToken
    return next()
})
